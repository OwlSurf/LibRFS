...
struct slot_record {
    uint32_t seq;      /* monotonic sequence — used on overflow recovery */
    uint32_t event_id;
    uint8_t  pad[7];
    uint8_t  status;   /* last byte is reserved: leave 0xFF on write */
};
...
int main(void)
{
    ...
    void *em = em_driver_init_((void *)nor_erase,
                               (void *)nor_read,
                               (void *)nor_write,
                               DEMO_SIZE,
                               DEMO_SECTOR_SIZE,
